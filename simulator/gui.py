...
BENCHMARK_METRICS = [
    ...
    {"id": "completed", "label": "完成任务数", "direction": "desc"},
    {"id": "unserved", "label": "未完成任务", "direction": "asc"},
    {"id": "overtime", "label": "超时任务", "direction": "asc"},
    {"id": "distance", "label": "总行驶距离", "direction": "asc"},
    {"id": "avg_response_time", "label": "平均响应时间", "direction": "asc"},
    {"id": "charging_wait", "label": "充电等待合计", "direction": "asc"},
]
...
def _run_single_simulation(payload: dict) -> dict:
    ...
    scenario = _build_dashboard_scenario(
        scale=scale,
    ...
    for event in normalized_events:
        ...
        for vehicle_id, route_nodes in event.route_by_vehicle.items():
            assigned_weight_default = float(task.weight) / max(1, len(event.vehicle_ids))
            assigned_weight = float(event.carried_weight_by_vehicle.get(int(vehicle_id), assigned_weight_default))
            route_items.append(
                {
                    "vehicle_id": int(vehicle_id),
                    "task_id": int(event.task_id),
                    "route_nodes": list(route_nodes),
                    "route_key": _route_key(route_nodes),
...
def _route_geometry(payload: dict) -> dict:
    ...
    if not isinstance(raw_points, list):
        raise ValueError("waypoints 必须是坐标数组。")
    waypoints: List[List[float]] = []
    for item in raw_points:
        if isinstance(item, (list, tuple)) and len(item) >= 2:
            waypoints.append([float(item[0]), float(item[1])])
    result = fetch_route_geometry(waypoints=waypoints, strategy=str(payload.get("route_strategy", "4")))
    return {
        "ok": True,
        ...
        "strategy": result.get("strategy", "4"),
    }
        ...
        "nodes": [
            {
                ...
                "x": node.x,
                "y": node.y,
                "label": str(node_meta.get(node.node_id, {}).get("name", "")),
                "address": str(node_meta.get(node.node_id, {}).get("address", "")),
            }
        ...
        "tasks": [
            {
                ...
                "weight": task.weight,
                "place_name": str(node_meta.get(task.node_id, {}).get("name", "")),
                "address": str(node_meta.get(task.node_id, {}).get("address", "")),
            }
            for task in scenario.tasks
        ],
        "vehicles": [
        ...
        "stations": [
            {
                ...
                "name": str(node_meta.get(station.node_id, {}).get("name", "")),
                "address": str(node_meta.get(station.node_id, {}).get("address", "")),
            }
...
def _load_benchmark_payload() -> dict:
    ...
    if not datasets and latest_path.exists() and latest_path.name.lower() not in seen_names:
        latest = _read_benchmark_dataset(key="latest", label="最新 summary.json", path=latest_path)
...
def _normalize_benchmark_row(row: object) -> dict | None:
    ...
    return {
        ...
        "charging_sessions": _safe_float(row.get("charging_sessions", 0.0)),
        "max_charging_wait": _safe_float(row.get("max_charging_wait", 0.0)),
        "collaborative_tasks": _safe_float(row.get("collaborative_tasks", 0.0)),
        "collaborative_task_ratio": _safe_float(row.get("collaborative_task_ratio", 0.0)),
        "max_vehicles_per_task": _safe_float(row.get("max_vehicles_per_task", 1.0)),
        "station_avg_utilization": row.get("station_avg_utilization", {}),
        "station_peak_utilization": row.get("station_peak_utilization", {}),
        "station_charging_sessions": row.get("station_charging_sessions", {}),
        "station_max_wait_time": row.get("station_max_wait_time", {}),
