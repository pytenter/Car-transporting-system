    ...
    def run(self, strategy: DispatchStrategy) -> Tuple[SimulationSummary, List[TaskResult], List[DispatchEvent]]:
        ...
        for now in range(simulation_end + 1):
            ...
            while pending and free_vehicle_ids:
                ...
                # Collaborative modeling: split the task load to each assigned vehicle by
                # capacity proportion, and record it for reporting/visualization.
                weight_by_vehicle: Dict[int, float] = {}
                if len(vehicle_ids) == 1:
                    weight_by_vehicle[vehicle_ids[0]] = float(task.weight)
                else:
                    cap_sum = sum(self.vehicles[vid].capacity for vid in vehicle_ids)
                    if cap_sum <= 1e-9:
                        break
                    assigned = 0.0
                    for idx, vid in enumerate(vehicle_ids):
                        if idx == len(vehicle_ids) - 1:
                            # Avoid floating drift so sum(weights)=task.weight
                            w = float(task.weight) - assigned
                        else:
                            w = float(task.weight) * (self.vehicles[vid].capacity / cap_sum)
                            assigned += w
                        weight_by_vehicle[vid] = max(0.0, w)

                missions: List[VehicleMission] = []
                mission_failed = False
                for vehicle_id in vehicle_ids:
                    carried_weight = weight_by_vehicle.get(vehicle_id, float(task.weight) / max(1, len(vehicle_ids)))
                    mission = self._plan_vehicle_mission(
                ...
                events.append(
                    DispatchEvent(
                        ...
                        carried_weight_by_vehicle={mission.vehicle_id: mission.carried_weight for mission in missions},
