    ...
    for i, task in enumerate(tasks):
        for k, vehicle in enumerate(vehicles):
            if not allow_collaboration and task.weight > vehicle.capacity + 1e-9:
                continue
            mission = _mission_from_depot(task, vehicle, scenario, depart_time=float(task.release_time))
            if mission is None:
                continue
...
def _traffic_multiplier(scenario: ScenarioData, when: float) -> float:
    for start, end, multiplier in scenario.config.rush_windows:
        if start <= when <= end:
            return multiplier
    return 1.0


def _mission_from_depot(
    task: Task,
    vehicle: Vehicle,
    scenario: ScenarioData,
    depart_time: float,
    ...
    if battery_at_start + 1e-9 >= direct_need + end_target:
        to_task_time = d_start_task / max(vehicle.speed, 1e-6) * _traffic_multiplier(scenario, depart_time)
        arrival_task_time = depart_time + to_task_time
        to_depot_time = d_task_depot / max(vehicle.speed, 1e-6) * _traffic_multiplier(
            scenario,
            arrival_task_time + scenario.config.service_time,
        )
        duration = to_task_time + scenario.config.service_time + to_depot_time
    ...
    for station_node, charge_rate in candidate_stations:
        ...
        to_station_time = d_start_station / max(vehicle.speed, 1e-6) * _traffic_multiplier(scenario, depart_time)
        arrival_station_time = depart_time + to_station_time
        finish_charge_time = arrival_station_time + charge_time

        to_task_time = d_station_task / max(vehicle.speed, 1e-6) * _traffic_multiplier(scenario, finish_charge_time)
        arrival_task_time = finish_charge_time + to_task_time

        to_depot_time = d_task_depot / max(vehicle.speed, 1e-6) * _traffic_multiplier(
            scenario,
            arrival_task_time + scenario.config.service_time,
        )
        duration = to_station_time + charge_time + to_task_time + scenario.config.service_time + to_depot_time
